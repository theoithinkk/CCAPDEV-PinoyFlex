...
const adminGetBadges = () => adminRequest(`/api/badges`);
const adminCreateBadge = (payload) => adminRequest(`/api/admin/badges`, { method: "POST", body: JSON.stringify(payload) });
const adminUpdateBadge = (key, payload) => adminRequest(`/api/admin/badges/${key}`, { method: "PATCH", body: JSON.stringify(payload) });
const adminDeleteBadge = (key) => adminRequest(`/api/admin/badges/${key}`, { method: "DELETE" });
...
function AdminPanel({ session, posts, setPosts, onToast, onRefreshPosts, adminGetUsers, adminModerateUser, adminDeleteUser, adminGetReports, adminUpdateReport, adminGetVerifications, adminUpdateVerification }) {
  ...
  const [newBadgeSubmitting, setNewBadgeSubmitting] = useState(false);
  const [editingBadgeKey, setEditingBadgeKey] = useState(null);
  const [editBadgeName, setEditBadgeName] = useState("");
  const [editBadgeDesc, setEditBadgeDesc] = useState("");
  const [editBadgeFile, setEditBadgeFile] = useState(null);
  const [editBadgeSubmitting, setEditBadgeSubmitting] = useState(false);
  ...
  useEffect(() => {
    ...
    let mounted = true;
    adminGetBadges()
      .then((data) => { if (mounted) setAdminBadges(data.badges || []); })
      .catch(() => { if (mounted) setAdminBadges([]); });
    return () => { mounted = false; };
  }, [tab]);
  ...
  async function handleDeleteBadge(key) {
    try {
      await adminDeleteBadge(key);
      setAdminBadges((prev) => prev.filter((b) => b.key !== key));
      onToast("Badge deleted.");
    } catch (err) { onToast(err?.message || "Failed to delete badge."); }
    setConfirmAction(null);
  }
  ...
  return (
    <div className="block" style={{ padding: "1.5rem" }}>
      ...
      <div className="profile-tabs" role="tablist" style={{ marginBottom: "1.5rem" }}>
        {["posts", "users", "reports", "verifications", "badges"].map((t) => (
...
{/* BADGES TAB */}
      {tab === "badges" && (
        <div>
          <div className="block" style={{ marginBottom: "2rem", padding: "1.5rem", background: "var(--bg-secondary)", borderRadius: "12px", border: "1px solid var(--border, #eaeaea)" }}>
            <h3 style={{ marginTop: 0, marginBottom: "1.5rem" }}>Create New Badge</h3>
            
            <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
              
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: "600", color: "var(--text-secondary, #4b5563)" }}>Badge Name</label>
                <input 
                  className="field-input" 
                  style={{ padding: "0.75rem 1rem", borderRadius: "8px", border: "1px solid var(--border, #d1d5db)", backgroundColor: "var(--bg-primary, #fff)", fontSize: "0.95rem", outline: "none", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}
                  placeholder="e.g., 500lb Deadlift" 
                  value={newBadgeName} 
                  onChange={e => {
                    setNewBadgeName(e.target.value);
                    setNewBadgeKey(e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-'));
                  }} 
                />
              </div>

              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: "600", color: "var(--text-secondary, #4b5563)" }}>Badge Key (Internal ID)</label>
                <input 
                  className="field-input" 
                  style={{ padding: "0.75rem 1rem", borderRadius: "8px", border: "1px solid var(--border, #d1d5db)", backgroundColor: "var(--bg-tertiary, #f3f4f6)", fontSize: "0.95rem", color: "var(--text-muted, #6b7280)", outline: "none" }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: "600", color: "var(--text-secondary, #4b5563)" }}>Description</label>
                <textarea 
                  className="field-textarea" 
                  style={{ padding: "0.75rem 1rem", borderRadius: "8px", border: "1px solid var(--border, #d1d5db)", backgroundColor: "var(--bg-primary, #fff)", fontSize: "0.95rem", fontFamily: "inherit", resize: "vertical", outline: "none", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                <label style={{ fontSize: "0.85rem", fontWeight: "600", color: "var(--text-secondary, #4b5563)" }}>Badge Image (Optional)</label>
                <label style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "1rem", border: "2px dashed var(--border, #d1d5db)", borderRadius: "8px", background: "var(--bg-primary, #fff)", cursor: "pointer" }}>
                  {newBadgeFile ? (
                    <img 
                      src={URL.createObjectURL(newBadgeFile)} 
                      alt="Preview" 
                      style={{ width: 48, height: 48, objectFit: "cover", borderRadius: "6px", border: "1px solid var(--border, #e5e7eb)" }} 
                    />
                  ) : (
                    <div style={{ width: 48, height: 48, borderRadius: "6px", background: "var(--bg-tertiary, #f3f4f6)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted, #9ca3af)", fontSize: "1.5rem" }}>+</div>
                  )}
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: "600", fontSize: "0.95rem", color: "var(--text-primary, #111)" }}>
                      {newBadgeFile ? newBadgeFile.name : "Click to choose an image"}
                    </div>
                    <div style={{ fontSize: "0.8rem", color: "var(--text-muted, #6b7280)" }}>JPG, PNG, GIF up to 5MB</div>
                  </div>
                  <input 
                    type="file" 
                    accept="image/*" 
              ...
              <button className="btn btn-primary" style={{ width: "fit-content", padding: "0.75rem 1.5rem", fontSize: "1rem", marginTop: "0.5rem" }} disabled={newBadgeSubmitting} onClick={async () => {
                ...
                try {
                  ...
                  setNewBadgeName(""); setNewBadgeKey(""); setNewBadgeDesc(""); setNewBadgeFile(null);
                  const b = await adminGetBadges(); setAdminBadges(b.badges || []);
                } catch(err) { onToast(err.message); }
                setNewBadgeSubmitting(false);
              }}>
                {newBadgeSubmitting ? "Creating..." : "Create Badge"}
              </button>
            </div>
          </div>
          ...
          <div className="profile-list">
            {adminBadges.length === 0 ? <p className="detail-muted">No badges found.</p> : adminBadges.map(b => (
              <div key={b.key} className="profile-item" style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                {b.imageUrl ? (
                  <img src={b.imageUrl} alt={b.name} style={{ width: 48, height: 48, borderRadius: '8px', objectFit: 'cover', marginTop: "4px" }} />
                ) : (
                  <div style={{ width: 48, height: 48, borderRadius: '8px', background: 'var(--bg-tertiary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', color: "var(--text-muted)", marginTop: "4px" }}>No Img</div>
                )}
                
                {editingBadgeKey === b.key ? (
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.75rem", background: "var(--bg-secondary, #f9fafb)", padding: "1rem", borderRadius: "8px", border: "1px solid var(--border, #e5e7eb)" }}>
                    <div style={{ fontWeight: "600", fontSize: "0.9rem", color: "var(--text-secondary, #4b5563)", marginBottom: "0.25rem" }}>
                      Editing: {b.key}
                    </div>

                    <input 
                      className="field-input" 
                      style={{ padding: "0.6rem 1rem", borderRadius: "6px", border: "1px solid var(--border, #d1d5db)", fontSize: "0.95rem" }} 
                      value={editBadgeName} 
                    ...
                    <textarea 
                      className="field-textarea" 
                      style={{ padding: "0.6rem 1rem", borderRadius: "6px", border: "1px solid var(--border, #d1d5db)", fontSize: "0.95rem", resize: "vertical" }} 
                      value={editBadgeDesc} 
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", marginTop: "0.25rem" }}>
                      <label style={{ fontSize: "0.85rem", fontWeight: "600", color: "var(--text-secondary, #4b5563)" }}>Replace Image (Optional)</label>
                      <label style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem", border: "1px dashed var(--border, #9ca3af)", borderRadius: "6px", background: "var(--bg-primary, #fff)", cursor: "pointer", transition: "background 0.2s" }}>
                        {editBadgeFile ? (
                          <img src={URL.createObjectURL(editBadgeFile)} alt="New Preview" style={{ width: 40, height: 40, objectFit: "cover", borderRadius: "4px", border: "1px solid var(--border)" }} />
                        ) : b.imageUrl ? (
                          <img src={b.imageUrl} alt="Current" style={{ width: 40, height: 40, objectFit: "cover", borderRadius: "4px", opacity: 0.6 }} />
                        ) : (
                          <div style={{ width: 40, height: 40, borderRadius: "4px", background: "var(--bg-tertiary)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", fontSize: "1.25rem" }}>+</div>
                        )}
                        <div style={{ flex: 1, fontSize: "0.85rem" }}>
                          <div style={{ fontWeight: "600", color: "var(--text-primary, #111)" }}>
                            {editBadgeFile ? editBadgeFile.name : "Click to upload a new image"}
                          </div>
                        </div>
                        {/* Hidden native input */}
                        <input type="file" accept="image/*" onChange={e => setEditBadgeFile(e.target.files[0] || null)} style={{ display: "none" }} />
                      </label>
                    </div>

                    <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
                      <button className="btn btn-primary" style={{ padding: "0.6rem 1.2rem", fontSize: "0.9rem" }} disabled={editBadgeSubmitting} onClick={async () => {
                        setEditBadgeSubmitting(true);
                        try {
                          ...
                          const refreshed = await adminGetBadges(); 
                          setAdminBadges(refreshed.badges || []);
                        } catch(err) { onToast(err.message); }
                        setEditBadgeSubmitting(false);
                      }}>{editBadgeSubmitting ? "Saving..." : "Save Changes"}</button>
                      <button className="btn btn-secondary" style={{ padding: "0.6rem 1.2rem", fontSize: "0.9rem" }} onClick={() => setEditingBadgeKey(null)}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <>
                    {/* STANDARD DISPLAY */}
                    <div style={{ flex: 1 }}>
                      <div className="profile-item-title">{b.name} <span className="detail-tag" style={{ marginLeft: "0.5rem" }}>{b.key}</span></div>
                      {b.description && <div className="profile-item-body">{b.description}</div>}
                    </div>
                    
                    {/* EDIT & DELETE BUTTONS */}
                    <div style={{ display: "flex", gap: "0.5rem", marginTop: "4px" }}>
                      <button className="btn btn-secondary" style={{ padding: "4px 10px", fontSize: "0.85rem" }} onClick={() => {
                        setEditingBadgeKey(b.key);
                        setEditBadgeName(b.name);
                        setEditBadgeDesc(b.description || "");
                        setEditBadgeFile(null);
                      }}>Edit</button>
                      <button className="btn btn-danger" style={{ padding: "4px 10px", fontSize: "0.85rem" }} onClick={() => setConfirmAction({
                        type: "delete_badge",
                        id: b.key,
                        label: `Permanently delete badge "${b.name}"?`
                      })}>Delete</button>
                    </div>
                  </>
                )}
          ...
          <div className="modal modal-confirm" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div className="modal-confirm-actions">
              ...
              <button className="btn btn-danger" onClick={() => {
                ...
                else if (confirmAction.type === "delete_badge") handleDeleteBadge(confirmAction.id);
              }}>Confirm</button>
            </div>
          </div>
        </div>
