...
const userBadges = await Badge.find({ key: { $in: user.badges || [] }, active: true }).lean();
    const populatedBadges = (user.badges || [])
      .map(badgeKey => {
...
router.post("/admin/badges", requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const exists = await Badge.findOne({ key });
    if (exists) return res.status(409).json({ error: "Badge key already exists." });
...
router.patch("/admin/badges/:key", requireAuth, requireAdmin, async (req, res, next) => {
  try {
...
router.delete("/admin/badges/:key", requireAuth, requireAdmin, async (req, res, next) => {
  try {
    const badge = await Badge.findOneAndDelete({ key: req.params.key });
    if (!badge) return res.status(404).json({ error: "Badge not found." });

    await User.updateMany(
      { badges: req.params.key },
      { $pull: { badges: req.params.key } }
    );
